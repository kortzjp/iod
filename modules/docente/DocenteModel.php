...
class DocenteModel extends DataBase {
    ...
    public function puntosDimensionUnico($docente, $dimension, $curso) {
        $this->query = "SELECT SUM(rp.respuesta) as 'obtenidos' "
                . " FROM respuestas rp, preguntas pr "
                . " WHERE rp.cursan IN ( "
                . " SELECT cr.id FROM cursan cr "
                . " WHERE cr.curso IN ( "
                . " SELECT cs.id FROM cursos cs "
                . " WHERE cs.docente = $docente AND cs.id=$curso)) "
                . " AND rp.pregunta = pr.idPregunta "
                . " AND pr.idDimension = $dimension";
    ...
    public function curso($id) {
        $this->query = "SELECT * FROM cursos WHERE id=$id LIMIT 1";
    ...
    public function getCursosUnico($id, $curso) {
        $this->query = "SELECT c.id, a.nombre, c.grupo "
                . " FROM asignaturas a, cursos c, cuatrimestres cu, cursan cn"
                . " WHERE a.id = c.asignatura AND c.docente = $id AND cu.estado = 1 AND c.id= $curso"
                . " AND cu.id=cn.cuatrimestre AND cn.curso= c.id GROUP BY cn.curso ORDER BY c.grupo LIMIT 1";
